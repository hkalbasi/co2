...
impl<A: TypeResolver> DoTransform for DeclarationSpecifier<A> {
    ...
    fn transform<B: Transformable<A>>(&self, b: &B) -> DeclarationSpecifier<B> {
        match self {
            ...
            DeclarationSpecifier::GNUAttribute(e) => DeclarationSpecifier::GNUAttribute(e.clone()),
        }
    }
}
                ...
                designator: designator
                    ...
                    .map(|m| match m {
                        crate::OffsetofMember::Field(name) => {
                            crate::OffsetofMember::Field(name.clone())
                        }
                        crate::OffsetofMember::Index(idx) => {
                            crate::OffsetofMember::Index(idx.transform(b))
                        }
                    })
                    .collect(),
            },
