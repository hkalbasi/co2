...
mod tests {
    ...
    fn macro_body<'a>(pp: &'a Preprocessor, name: &str) -> Option<&'a str> {
        pp.macros.get(name).map(|def| def.body.as_str())
    }

    #[test]
    fn set_target_aarch64_overrides_x86_and_long_double_macros() {
        let mut pp = Preprocessor::new();

        pp.set_target("aarch64");

        assert!(!pp.macros.is_defined("__x86_64__"));
        assert_eq!(macro_body(&pp, "__aarch64__"), Some("1"));
        assert_eq!(macro_body(&pp, "__CHAR_UNSIGNED__"), Some("1"));
        assert_eq!(macro_body(&pp, "__LDBL_MANT_DIG__"), Some("113"));
        assert_eq!(macro_body(&pp, "__LDBL_DECIMAL_DIG__"), Some("36"));
        assert_eq!(macro_body(&pp, "LDBL_DIG"), Some("33"));
    }

    #[test]
    fn set_target_riscv64_overrides_x86_and_long_double_macros() {
        let mut pp = Preprocessor::new();

        pp.set_target("riscv64");

        assert!(!pp.macros.is_defined("__x86_64__"));
        assert!(!pp.macros.is_defined("__SSE__"));
        assert_eq!(macro_body(&pp, "__riscv"), Some("1"));
        assert_eq!(macro_body(&pp, "__riscv_xlen"), Some("64"));
        assert_eq!(macro_body(&pp, "__LDBL_DIG__"), Some("33"));
        assert_eq!(macro_body(&pp, "DECIMAL_DIG"), Some("36"));
    }

    #[test]
    fn set_target_i686_keeps_ilp32_and_enables_backend_sse_macros() {
        let mut pp = Preprocessor::new();
        ...
        assert!(!pp.macros.is_defined("__x86_64__"));
        assert!(!pp.macros.is_defined("__SSE__"));
        assert_eq!(macro_body(&pp, "__i386__"), Some("1"));
        assert_eq!(macro_body(&pp, "__SIZEOF_POINTER__"), Some("4"));
        assert_eq!(macro_body(&pp, "__SIZEOF_LONG_DOUBLE__"), Some("12"));
        assert_eq!(macro_body(&pp, "__LONG_WIDTH__"), Some("32"));

        pp.set_sse_macros(false);

        assert_eq!(macro_body(&pp, "__SSE__"), Some("1"));
        assert_eq!(macro_body(&pp, "__SSE2__"), Some("1"));
        assert_eq!(macro_body(&pp, "__MMX__"), Some("1"));
        assert!(!pp.macros.is_defined("__SSE_MATH__"));
        assert!(!pp.macros.is_defined("__SSE2_MATH__"));
    }
}
