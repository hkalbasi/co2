...
use co2_parser::{
    Constant, Declaration, Expression, InitDeclarator, Item, LazyCompoundStatement, RustPath,
    RustType, Span, Spanned, State, Statement, StatementOrDeclaration, parse_compound_statement,
    parse_items,
};
    ...
    for stmt in compound.statements {
        match stmt.0 {
            StatementOrDeclaration::Statement(s) => {
                lower_stmt(s.0, &mut locals, &mut locals_map, &mut ops)?
            }
            StatementOrDeclaration::Declaration(d) => {
                lower_decl(d.0, resolver, &mut locals, &mut locals_map, &mut ops)?
            }
        }
    }
...
fn lower_stmt(
    ...
    match stmt {
        Statement::Return(expr) => {
            if let Some(expr) = expr {
                match expr.0 {
                    ...
                    other => {
                        let (operand, mut pre) = lower_expr((other, expr.1), locals, locals_map)?;
                        ops.append(&mut pre);
                        ops.push(MirOp::Assign {
                            dst: 0,
                            src: operand,
        ...
        Statement::Expression(expr) => match expr.0 {
            Expression::Call { func, params } => {
                let (args, mut pre) = lower_call_args(params, locals, locals_map)?;
                ops.append(&mut pre);
                let callee = lower_callee(func.0)?;
